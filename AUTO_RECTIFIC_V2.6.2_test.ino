...
#define KL2_OFF 500    // ВРЕМЯ РАБОТЫ КЛАПАНА ОТБОРА ТЕЛА (millis)
//СЧЕТЧИКИ ВРЕМЕНИ
...
bool tflag2 = 0;        // ФЛАГ ФИКСАЦИИ ТЕМПЕРАТУРЫ УО 2
bool xflag = 0;         // ФЛАГ ЗАВЫШЕНИЯ ТЕМПЕРАТУРЫ УО
bool xflag2 = 0;        // ФЛАГ ЗАВЫШЕНИЯ ТЕМПЕРАТУРЫ УО 2
...
if (mode == 3 && uo_temp > 70 && count_self >= TSELF && count_head >= head_time && !tflag) {
...
if ((millis() - tmr_kl2_body >= KL2_PER) && (uo_temp >= (uo_temp_fix + DELT)) && !xflag) {
  kl2_off = kl2_off - DECR;
  xflag = 1;
  xflag_counter = xflag_counter + 1;
}
